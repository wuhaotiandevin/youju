...
class MainController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $user[0]= Db::name("user")->where('user_type','>',1)->count();
        $user[1]= Db::name("user")->where('user_type',3)->count();
        $user[2]= Db::name("user")->where('user_type',2)->count();
        $user[3]= Db::name("user")->where('user_type','>',1)->whereTime('create_time','yesterday')->count();
        $this->assign('user', $user);

        $money[0]= Db::name("user_moneyrecord")->where(['style'=>'充值','status'=>1])->sum('money');
        $money[1]= Db::name("user_moneyrecord")->where('style','提现')->sum('money');
        $money[2]= $money[0] - $money[1];
        $money[3]= Db::name("user_moneyrecord")->where(['style'=>'充值','status'=>1])->whereTime('pay_time','yesterday')->sum('money');
